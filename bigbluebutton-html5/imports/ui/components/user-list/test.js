...
const normalizeRaiseHandName = emoji => (
  emoji in RAISE_HAND ? RAISE_HAND[emoji] : emoji
);
...
const setRaiseHandStatus = (userId, emoji) => {
  const statusAvailable = (Object.keys(RAISE_HAND).includes(emoji));

  return statusAvailable
    ? makeCall('setRaiseHandStatus', Auth.userID, emoji)
    : makeCall('setRaiseHandStatus', userId, 'none');
};
...
export default {
  ...
  getRaiseHandList: () => RAISE_HAND
  getEmoji: () => Users.findOne({ userId: Auth.userID }, { fields: { emoji: 1 } }).emoji,
  getRaiseHand: () => Users.findOne({ userId: Auth.userID }, { fields: { emoji: 1 } $
